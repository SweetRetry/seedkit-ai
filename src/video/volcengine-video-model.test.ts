...
import { VolcengineVideoModel } from './volcengine-video-model';
...
function createMockFetch(
  ...
  statusBody: unknown = {
    ...
    status: 'succeeded',
    content: { video_url: 'https://cdn.volcengine.com/output.mp4' },
    seed: 42,
    resolution: '1080p',
    ratio: '16:9',
    duration: 5,
...
function createModel(fetchMock?: ReturnType<typeof vi.fn>, modelId = 'doubao-seedance-1-0-pro-250528') {
  return new VolcengineVideoModel(modelId, {
...
describe('VolcengineVideoModel', () => {
  describe('constructor', () => {
    it('exposes correct provider, modelId, specificationVersion', () => {
      const model = createModel();
      expect(model.provider).toBe('volcengine.video');
  ...
  describe('doGenerate – provider options', () => {
    it('sends resolution option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { resolution: '1080p' } },
      });
    ...
    it('sends ratio from provider options (overrides aspectRatio)', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        aspectRatio: '16:9',
        providerOptions: { volcengine: { ratio: '9:16' } },
      });
    ...
    it('sends watermark option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { watermark: true } },
      });
    ...
    it('sends generate_audio option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { generate_audio: false } },
      });
    ...
    it('sends camera_fixed option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { camera_fixed: true } },
      });
    ...
    it('sends return_last_frame option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { return_last_frame: true } },
      });
    ...
    it('sends service_tier option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { service_tier: 'flex' } },
      });
    ...
    it('sends draft option', async () => {
      ...
      await createModel(fetch, 'doubao-seedance-1-5-pro-251215').doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { draft: true } },
      });
    ...
    it('sends frames option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { frames: 125 } },
      });
    ...
    it('sends execution_expires_after option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: { volcengine: { execution_expires_after: 7200 } },
      });
    ...
    it('sends callback_url option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: {
          volcengine: { callback_url: 'https://example.com/webhook' },
        },
      });
    ...
    it('does not send pollIntervalMs or pollTimeoutMs to API', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...defaultOptions,
        providerOptions: {
          volcengine: { pollIntervalMs: 1000, pollTimeoutMs: 30000 },
        },
      });
  ...
  describe('doGenerate – headers', () => {
    it('merges provider headers and per-request headers', async () => {
      const fetch = createMockFetch({ id: 'task-id-123' });
      const model = new VolcengineVideoModel('doubao-seedance-1-0-pro-250528', {
        provider: 'volcengine.video',
        baseURL: BASE_URL,
        headers: () => ({
  ...
  describe('doGenerate – response', () => {
    it('returns video with correct url and mediaType', async () => {
      ...
      expect(result.videos[0]).toStrictEqual({
        type: 'url',
        url: 'https://cdn.volcengine.com/output.mp4',
        mediaType: 'video/mp4',
      });
    });
    ...
    it('includes last_frame_url in providerMetadata when present', async () => {
      const fetch = createMockFetch(
        { id: 'task-id-123' },
        {
          ...
          status: 'succeeded',
          content: {
            video_url: 'https://cdn.volcengine.com/output.mp4',
            last_frame_url: 'https://cdn.volcengine.com/last-frame.png',
          },
        },
      );
      const result = await createModel(fetch).doGenerate({ ...defaultOptions });

      expect(result.providerMetadata?.volcengine?.last_frame_url).toBe(
        'https://cdn.volcengine.com/last-frame.png',
      );
    });
    ...
    it('does not include service_tier in providerMetadata', async () => {
      ...
      expect(result.providerMetadata?.volcengine).not.toHaveProperty(
        'service_tier',
      );
    });
    ...
    it('throws when task status is failed', async () => {
      ...
      await expect(
        createModel(fetch).doGenerate({
          ...defaultOptions,
          providerOptions: { volcengine: { pollIntervalMs: 1 } },
        }),
      ).rejects.toMatchObject({
  ...
  describe('doGenerate – polling', () => {
    ...
    it('times out after pollTimeoutMs', async () => {
      ...
      await expect(
        createModel(fetch).doGenerate({
          ...defaultOptions,
          providerOptions: { volcengine: { pollIntervalMs: 10, pollTimeoutMs: 50 } },
        }),
      ).rejects.toMatchObject({
